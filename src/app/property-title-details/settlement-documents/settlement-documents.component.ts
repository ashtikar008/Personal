...
export class SettlementDocumentsComponent implements OnInit {
  settlementDocuments:FormGroup;
  ...
  constructor(public fb: FormBuilder,private apiService:ApiService,private toastrService: ToastrService) { 
    this.settlementDocuments = this.fb.group({
      RTF_SUBSCRIPTION: new FormControl('', [Validators.required])
    });
          ...
          switch (event.type) {
            ...
            case HttpEventType.Response:
              ...
              if (fieldName == 'additionalDoc') {
                ...
                this.settlementDocuments.patchValue({
                  [fieldName]: event.body.items[0].documentId
                });
              }
              console.log(this.settlementDocuments.value)
              setTimeout(() => {
                this.progress = 0;
              }, 1500);
          }
